...
public class OWaveInfoDialog extends WaveInfoDialog {
    ...
    void buildGroups(){
        TextFieldFilter fieldFilter = (field, c) -> {
            if (!(Character.isDigit(c) || c == '-')) return false;

            String text = field.getText() + c;
            if (text.length() > 0) {
                if (text.indexOf("-") > 0) return false;
                if (text.chars().filter(c2 -> c2 == '-').count() > 1) return false;
                if (text.charAt(0) == '0' && text.chars().filter(c2 -> c2 == '0').count() > 1) return false;
            }

            return true;
        };

        TextFieldFilter fieldFloatFilter = (field, c) -> {
            ...
            if (text.length() > 0) {
                ...
                if (text.chars().filter(c2 -> c2 == '-').count() > 1) return false;
                if (text.length() > 1) {
                    if (text.charAt(0) == '0' && text.charAt(1) != '.') return false;
                }
            }
            ...
            for(SpawnGroup group : groups){
                table.table(Tex.button, t -> {
                    ...
                    t.button(b -> {
                        ...
                        String name = group.type.localizedName;
                        if (name.length() > 12) name = name.substring(0, 9) + "...";
                        b.add(name).color(Pal.accent);

                        b.add().growX();

                        
                        b.label(() -> { // AUGHHH
                            if (group.begin + 1 > 999) return "999+";
                            else if (group.begin + 1< -999) return "-999";
                            else return (group.begin + 1) + "";
                    ...
                    if(expandedGroup == group){
                        t.table(spawns -> {
                            spawns.field("" + (group.begin + 1), fieldFilter, text -> {
                            ...
                            spawns.field(group.end == never ? "" : (group.end + 1) + "", fieldFilter, text -> {
                        ...
                        t.table(a -> {
                            ...
                            a.add(" + ");
                            a.field(Strings.fixed((Mathf.zero(group.unitScaling) ? 0 : 1f / group.unitScaling), 2), fieldFloatFilter, text -> {
                        ...
                        t.table(a -> {
                            a.field((int)group.shields + "", fieldFilter, text -> {
                                if(Strings.canParseInt(text)){
                            ...
                            a.add(" + ");
                            a.field((int)group.shieldScaling + "", fieldFilter, text -> {
                                if(Strings.canParseInt(text)){
                            ...
                            a.button("", () -> {
                                ...
                                dialog.cont.pane(p -> {
                                    ...
                                    for(var spawn : spawner.getSpawns()){
                                        ...
                                        if(i >= 20){
                                            break;
                                        } NO, MORE SPAWNS TO THE GOD OF SPAWNS! */
                                    }
            ...
            p.table(ut -> {
                ut.table(t -> {
                    ...
                    for(UnitType type : content.units()){
                        ...
                        t.button(t2 -> {
                            t2.left();
                            t2.image(type.uiIcon).size(8 * 4).scaling(Scaling.fit).padRight(2f);
                        }, () -> {
                            lastType = type;
                            group.type = type;
                            dialog.hide();
                            buildGroups();
                        }).tooltip(type.localizedName).pad(2).margin(12f);
                        if(++i % 8 == 0) t.row();
                    }
                });
    
                ut.row();
                ut.add("@waves.hiddenunits").color(Pal.accent).padTop(10f).padBottom(4f).center();
                ut.row();
                ut.image().color(Pal.accent).height(3f).padBottom(10f).fillX();
                ut.row();
    
                ut.table(t -> {
                    int i = 0;
                    for(UnitType type : content.units()){
                        if (type == UnitTypes.block || type.localizedName.equals("turret-unit-build-tower")) continue; // game crashing
                        if (!type.isHidden()) continue;
                        t.button(t2 -> {
                            t2.left();
                            t2.image(type.uiIcon).size(8 * 4).scaling(Scaling.fit).padRight(2f);
        ...
        dialog.cont.pane(p -> {
            ...
            for(StatusEffect effect : content.statusEffects()){
                p.button(t -> {
                    ...
                    // TODO: implement invincible status effect icon

                    if(effect != StatusEffects.none){
                        t.add(effect.localizedName.substring(0,1).toUpperCase() + effect.localizedName.substring(1));
                    }else{
